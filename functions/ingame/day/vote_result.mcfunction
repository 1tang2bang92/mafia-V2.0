...
execute if score voteCount master matches 1 run tellraw @a ["",{"selector":"@p[scores={result-vote=0}]"},{"text":" has been voted for execution."}]
execute if score voteCount master matches 1 if entity @p[tag=player,scores={result-vote=0},gamemode=adventure] as @p[tag=player,scores={result-vote=0}] run function mafia:ingame/day/execution
execute if score voteCount master matches 1 run function mafia:main/time/set_twilight
