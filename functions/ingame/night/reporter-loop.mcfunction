...
execute if score @s master matches 1..6 positioned ~ ~-1 ~ as @p[tag=death,scores={job-reporter=0},distance=..1,limit=1,sort=nearest] run tag add @s reporter-target
execute if score @s master matches 1..6 positioned ~ ~-1 ~ as @p[tag=death,scores={job-reporter=0},distance=..1,limit=1,sort=nearest] run tag @p[scores={job-doctor=1}] add used-abil
