...
tellraw @p[scores={job-police=1},tag=!used-abil] ["",{"text":"[Mafia V2.0] ","color":"aqua","bold":true,"insertion":"/tellraw @p %s"},{"selector":"@p[tag=police-target]","color":"blue","bold":false},{"text":"를 조사하시겠습니까?","color":"gold"},{"text":" [예]","color":"green","clickEvent":{"action":"run_command","value":"/function mafia:ingame/night/imprisonment"}}]
