...
execute if score @s master matches ..6 positioned ~ ~-1 ~ if entity @p[team=Citizen,tag=player,distance=..1,limit=1,sort=nearest,scores={job-spy=0}] at @p[scores={job-spy=0},sort=nearest,team=Citizen] as @s run function mafia:ingame/night/spy-fail
